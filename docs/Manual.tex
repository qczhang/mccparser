...
%% Last Modified On : Wed May  9 15:47:12 2001
%% Update Count     : 9
%% Status           : Ok.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\begin{itemize}
\item residue, connect and pair statements must contain at least one
  definition;
\item sampling size for residue, connect and pair statements can be specified
  ...
  of whole set;
\item sampling\_\,factor statement added, apply a real factor to sampling sizes 
  for every residue, connect and pair statements specified below this statement; 
...
\begin{grammarrule}{statement}{\terminal{sequence} \terminal{(} \token{IDENT}
  ...
  \altrule{\terminal{sampling\_\,factor} \terminal{(} \token{FLOAT} \terminal{)}}
  \altrule{\terminal{explore} \terminal{(} \nonterminal{fgRef}
    \nonterminal{expfile}? \terminal{)}}
  \altrule{\terminal{exploreLV} \terminal{(} \nonterminal{fgRef}
    \nonterminal{expfile}? \nonterminal{backtracksize}? \nonterminal{timelimit\_\,exp}? \terminal{)}}
...
\begin{grammarrule}{pairdef}{\nonterminal{residueRef} \nonterminal{residueRef}
  ...
  \altrule{\nonterminal{residueRef} \nonterminal{queryexp} \nonterminal{residueRef}
    \nonterminal{queryexp} \nonterminal{queryexp} \nonterminal{sampling}}
\end{grammarrule}
\begin{grammarrule}{sampling}{\token{INTEGER} \token{PERCENT}}
  \altrule{\token{INTEGER}}
\end{grammarrule}
\begin{grammarrule}{expfile}{\terminal{fileName\_\,pdb} \terminal{(}
    \token{STRING} \terminal{)} \terminal{zipped}?}
\end{grammarrule}
\begin{grammarrule}{backtracksize}{\terminal{backtrack\_\,size} \terminal{(}
    \token{INTEGER} \token{INTEGER} \terminal{)}}
\end{grammarrule}
\begin{grammarrule}{timelimit\_\,exp}{\terminal{time\_\,limit} \terminal{(}
