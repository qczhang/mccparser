...
%% Last Modified On : Fri Sep  1 13:08:52 2000
%% Update Count     : 4
%% Status           : Ok.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\noindent To include the parser in an application, you must follow these
instructions:
\begin{itemize}
...
\item assign to the boolean variable {\tt interactive\_parser} the true
  ...
  parsed;
\item assign to {\tt yyin} the {\tt FILE*} of the input; assign stdin for
  interactive input or the result of the {\tt fopen} call on the input file;
\item use {\tt yyparse ()} to parse the input.  The result can be found
...
\item define a function on each {\tt MccPStruc} to evaluate the abstract
  syntax tree;
\item link your application with the mccparser and the flex librares: {\tt
    -lmccparser} and {\tt -lfl}.
\end{itemize}
